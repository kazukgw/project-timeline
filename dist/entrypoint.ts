...
function getAllResources(sheetName) {
    ...
    return JSON.stringify({
        groups: groups.map((g: TableRecord)=>{return g.values}),
        projects: projects.map((p: TableRecord)=>{return p.values}),
        schedules: schedules.map((s: TableRecord)=>{return s.values}),
    })
}
...
function updateSchedule(scheduleJson) {
    ...
    let record = app.scheduleTable.findRecordByPrimaryKey(schedule[app.scheduleTable.primaryKey]);
    record.values.start = moment(schedule.start.replace('Z', ''), moment.HTML5_FMT.DATETIME_LOCAL_MS).add(9, 'hours').format('YYYY/MM/DD');
    record.values.end = moment(schedule.end.replace('Z', ''), moment.HTML5_FMT.DATETIME_LOCAL_MS).add(9, 'hours').format('YYYY/MM/DD');
...
class Config {
    ...
    constructor(sheetName: string, headerRangeFirstRowNumber: number, recordRangeFirstRowNumber: number, primaryKey: string) {
        let configTable = new Table(sheetName, headerRangeFirstRowNumber, recordRangeFirstRowNumber, primaryKey);
        let records = configTable.getAllRecords();
        var configData = {};
        records.forEach((record: TableRecord, index: number)=>{
            Logger.log(`Config record: record.values: ${JSON.stringify(record.values)}`);
        ...
        this.groupSheetHeaderRangeFirstRowNumber = configData['groupSheetHeaderRangeFirstRowNumber'];
        this.groupSheetRecordRangeFirstRowNumber = configData['groupSheetRecordRangeFirstRowNumber'];
        this.groupPrimaryKey = configData['groupPrimaryKey'];

        this.projectSheetName = configData['projectSheetName'];
        this.projectSheetHeaderRangeFirstRowNumber = configData['projectSheetHeaderRangeFirstRowNumber'];
        this.projectSheetRecordRangeFirstRowNumber = configData['projectSheetRecordRangeFirstRowNumber'];
        this.projectPrimaryKey = configData['projectPrimaryKey'];

        this.scheduleSheetName = configData['scheduleSheetName'];
        this.scheduleSheetHeaderRangeFirstRowNumber = configData['scheduleSheetHeaderRangeFirstRowNumber'];
        this.scheduleSheetRecordRangeFirstRowNumber = configData['scheduleSheetRecordRangeFirstRowNumber'];
