...
class Table {
    ...
    constructor(sheetName: string, headerRangeFirstRowNumber: number, recordRangeFirstRowNumber: number, primaryKey: string) {
            ...
            // header の数は最大 20
            let headers = <Array<string>>this.sheetObj.getRange(headerRangeFirstRowNumber, 1, 1, 20).getValues()[0];
            this.headers = [];
    ...
    private getRangeByRowNumber(rowNumber: number): GoogleAppsScript.Spreadsheet.Range {
        return this.sheetObj.getRange(rowNumber, 1, 1, this.headers.length);
    }

    private findRangeByPrimaryKey(key: string): GoogleAppsScript.Spreadsheet.Range {
    ...
    private getPrimaryKeyColRange(): GoogleAppsScript.Spreadsheet.Range {
        return this.sheetObj.getRange(this.recordRangeFirstRowNumber, this.primaryKeyColumnNumber,
...
class TableRecord {
    ...
    constructor(table: Table, range: GoogleAppsScript.Spreadsheet.Range) {
        ...
        let values = {};
        this.table.headers.forEach((k: string, i: number)=>{ values[k] = rangeVals[i]});
        this.values = values;
    }
    ...
    private valueArray(): Array<any>{
        ...
        this.table.headers.forEach((k: string)=>{ valueAry.push(values[k]); });
